...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
	...
	<div class="ms-auto">
		<div class="btn-group">
			...
			<a href="{{ route('admin.permission.index')}}" class="btn btn-primary d-none d-sm-inline-block" type="button" class="btn btn-primary"> View All Permission </a>
	...
	<div class="col-xl-12 mx-auto">
		<div class="card">
			<div class="card-header px-4 py-3">
				<h5 class="mb-0">Edit Permission Form</h5>
			</div>
			<div class="card-body p-4">

				
				<form  id="category-form" class="row g-3" novalidate="" method="POST" action="{{ route('admin.permission.update', $permission->id )}}">
					...
					<div class="col-md-12 {{ $errors->has('name') ? 'is-invalid' : '' }}">
						...
						placeholder="Permission Name" value="{{ $permission->name }}" id="name"  >
					</div>
					@if($errors->has('name'))
					...
					<div class="col-md-12 {{ $errors->has('slug') ? 'is-invalid' : '' }}">
						...
						<input type="text"  name="slug" class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" 
						        placeholder="Permission Slug" value="{{ $permission->slug }}" id="slug"  >
					</div>
					...
					<div class="col-md-12 {{ $errors->has('display_name') ? 'is-invalid' : '' }}">
						...
						<input type="text"  name="display_name" class="form-control {{ $errors->has('display_name') ? 'is-invalid' : '' }}" 
						        placeholder="Display Name" value="{{ $permission->display_name }}" id="displayName"  >
					</div>
					...
					<div class="col-md-12 {{ $errors->has('description') ? 'is-invalid' : '' }}">
						...
						<textarea rows="3"  name="description" class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" 
						        placeholder="Description" value="{{ $permission->description }}" id="description"  >
						</textarea>		
					</div>
